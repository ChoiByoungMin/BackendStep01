...
/*
 * 
 * DispatchServlet은 Spring에서 사용하는 DesignPattern을 사용한 클래스 명칭디다.
 * 이 역할은 *.do로 들어오는 모든 주소를 일단 받아서 분기 시켜주는 역할이다.
...
public class DispatchServlet extends HttpServlet {
	...
	// get요청/post요청을 모두 받기위해 구현하는 메서드
	@Override
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		System.out.println("DisptchServlet::service() - servletPath=" + servletPath);
		
		try {
			String pageControllerPath = null;
			
			
			// 1. 주소에 따라 처리할 pagetCsontrollerPath를 지정한다.
			// 2. 주소에 따라 약속된 parameter를 꺼내서 requeest공유공간에 저장한다.
			
			/*
			 * 현재 각 pageController에 해당하는 Servlet클래스들은
			 * request공간에서 Parameter를 꺼내도록 구현되어 있다.
			 * 하지만 Servlet의 종속성을 약화하고 보다 독립성을 가와하기 위해
			...
			// pageControllerPath가 지정햏준 viewUrl값을 꺼내서 화면 생성을 맡긴다
			...
			if(viewUrl.startsWith("redirect:")) {
				...
				// 브라우저는 redirect 요청을 보내다.
				// 브라우저는 redirect요청을 받으면 해당 url로 새롭게 다시 접속한다.
			...
			RequestDispatcher rd = req.getRequestDispatcher("/Errpr.jsp");
			rd.forward(req, resp);
		}
	}
}
